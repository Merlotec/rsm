...
def run_logic(model, dist_model, fall_model, dir_sign):
    ...
    while True:
        if state is 0:
            ...
            sim = simulate_imm(rects[0], rects[1], model, dist_model)
            if sim:
                (pr_t, pr_d) = sim
                pr_b = pr_t * (np.pi * 2.0) / tau_b
                loc_d = (pr_d) % (np.pi * 2.0)
                pr_fall_out = fall_model.predict(np.array([[tau_b, loc_d]]))[0][0]
                moves = play(pr_fall_out, 14)
                print(f"Play: {moves}")
                beep_after_x_seconds(pr_t.item(), loc_d)
                state = 2
            ...
            if num != newnum:
                ...
                if moves.contains(pos):
                    ...
                    print(f"WE WIN!!!!! ROI: {roi}")
                else:
                    print("WE LOSE :(")

                print(f"Total ROI: {total_roi}, Log total ROI: {np.log(total_roi)}")
                fall = dis_to_fall(pos_to_dis(pos), pr_b, pr_d, dir_sign)
                fall_n = round(fall)
                add_training_fall_data(fall_model, np.array([[tau_b, loc_d]]), np.array([fall_n]), 'fall.h4') # Train the model
                state = 0
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            run_logic(model, dist_model, fall_model, -1)
        elif key == ord('s'):
